    ...
    <header className="shrink-0 sticky top-0 z-10 flex items-center justify-between h-16 px-6 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center">
          <button
            onClick={toggleRole}
            aria-label={`Current role is ${role}. Switch to ${role === 'Viewer' ? 'Admin' : 'Viewer'} mode`}
            title={`Switch to ${role === 'Viewer' ? 'Admin' : 'Viewer'} mode`}
        ...
        <div className="flex items-center">
          <button title="My Profile" aria-label="My Profile" className="flex items-center justify-center p-1 text-gray-500 transition-colors rounded-full cursor-pointer hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
